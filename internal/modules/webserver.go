...
func newWebServerProbeClient(cfg *core.Config) *http.Client {
	client := core.NewHTTPClient(cfg)
	// Preserve the first response in a redirect chain. Redirecting edge
	// servers often expose the only useful Server/Via fingerprint, while the
	// final application or access-denied page suppresses it.
...
func RunWebServer(cfg *core.Config, report *core.ReconReport, log *core.Logger) {
	...
	client := newWebServerProbeClient(cfg)
